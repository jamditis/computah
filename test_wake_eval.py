...
def _row(threshold, fr_per_act, fa_per_hour) -> ThresholdRow:
    ...
    return ThresholdRow(
        threshold=threshold, activations=10, false_rejects=0,
        false_rejects_per_activation=fr_per_act, false_accepts=0,
        nonwake_hours=1.0, false_accepts_per_hour=fa_per_hour)
...
def test_threshold_grid() -> None:
    ...
    check("uneven step never exceeds the requested max",
          uneven[-1] == 0.8, f"last={uneven[-1]}")

    for bad, why in (((0.1, 0.9, 0.0), "zero step"), ((0.1, 0.9, -0.1), "negative step"),
...
def test_count_false_accepts() -> None:
    ...
    check("no crossing is no fire",
          wake_eval.count_false_accepts([0.1, 0.2, 0.1], 0.5, 3) == 0, "0")
    check("gap of 1 counts every high frame",
          wake_eval.count_false_accepts([0.9, 0.9, 0.9], 0.5, 1) == 3, "3")
    fires = wake_eval.count_false_accepts([0.9, 0.9, 0.9, 0.9, 0.9], 0.5, 3)
    check("a sustained run counts once per refractory window", fires == 2,
          f"5 highs, gap 3 -> {fires}")
    check("separated fires each count",
          wake_eval.count_false_accepts([0.1, 0.9, 0.1, 0.9], 0.5, 1) == 2, "2")
...
def test_sweep_false_rejects() -> None:
    ...
    r = rows[0]
    check("half the activations fall below 0.5",
    ...
    check("all reject above every peak",
...
def test_sweep_false_accepts_per_hour() -> None:
    ...
    check("2 fires over 1 hour is 2.0/hour",
    ...
    check("no non-wake audio -> 0.0/hour, not a crash",
    ...
    check("a fire with zero measured duration is inf, never a safe 0",
    ...
    check("refractory 0 suppresses nothing (every crossing counts)",
          none[0].false_accepts == 3, f"fa={none[0].false_accepts}")
    try:
...
def test_latency_tolerance() -> None:
    ...
    check("a fire later than tolerance is a reject",
    ...
    check("a fire within tolerance is accepted",
          lenient[0].false_rejects_per_activation == 0.0,
          f"rate={lenient[0].false_rejects_per_activation}")

    unmeasured = wake_eval.sweep([Activation(0.9, latency_s=None)], [], [0.5],
                                 latency_tolerance_s=0.3)
    check("unmeasured latency is judged on peak alone",
...
def test_recommend() -> None:
    ...
    check("within budget, pick lowest false-reject",
          rec.threshold == 0.5 and rec.meets_budget, f"{rec.threshold} {rec.reason}")

    tight = wake_eval.recommend(rows, max_false_accepts_per_hour=0.1)
    check("tighter budget forces the higher threshold",
          tight.threshold == 0.7 and tight.meets_budget, f"{tight.threshold}")

    ties = [_row(0.6, 0.2, 0.0), _row(0.8, 0.2, 0.0)]
    tie = wake_eval.recommend(ties, max_false_accepts_per_hour=1.0)
    check("equal false-rejects tie-break to the lower threshold",
          tie.threshold == 0.6, f"{tie.threshold}")

    over = [_row(0.3, 0.0, 5.0), _row(0.5, 0.1, 3.0), _row(0.7, 0.4, 9.0)]
    miss = wake_eval.recommend(over, max_false_accepts_per_hour=0.0)
    check("nothing in budget -> flag it and give the lowest false-accept rate",
          miss.threshold == 0.5 and not miss.meets_budget, f"{miss.threshold} {miss.reason}")
