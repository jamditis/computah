...
def main() -> int:
    ...
    check("Piper speech is confirmed as speech",
          pipeline._confirm_speech(speech, thr), f"peak={sp_peak:.3f} >= {thr}")
    ...
    check("white noise is rejected (not speech)",
          not pipeline._confirm_speech(noise, thr), f"peak={n_peak:.3f} < {thr}")
    ...
    check("a 440 Hz tone is rejected (not speech)",
          not pipeline._confirm_speech(tone, thr), f"peak={t_peak:.3f} < {thr}")

    silence = np.zeros(16000, dtype=np.int16)
    s_peak = peak_prob(silence)
    check("silence is rejected (not speech)",
          not pipeline._confirm_speech(silence, thr), f"peak={s_peak:.3f} < {thr}")
    ...
    check("speech clears the loudest non-speech case by a wide margin",
