...
def main() -> int:
    ...
    pipeline.capture_request = lambda fr, preroll=None: np.zeros(0, dtype=np.int16)
    ...
    real_cap2, real_tx2, real_brain2 = (
        pipeline.capture_request, pipeline.transcribe_detailed, pipeline.brain)
    pipeline.capture_request = lambda fr, preroll=None: np.full(8 * FRAME_SIZE, 4000, dtype=np.int16)
