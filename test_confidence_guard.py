...
def main() -> int:
    ...
    check(
        ...
        not ok and reason == "empty transcript",
        f"ok={ok} reason={reason!r}",
    )

    ok, reason = confident(Transcript("   ", -0.1, 0.01))
    check(
        "whitespace-only transcript is rejected",
        not ok and reason == "empty transcript",
        f"ok={ok} reason={reason!r}",
    )
    ...
    check(
        "low avg_logprob is rejected as low confidence",
    ...
    check(
        "high no_speech_prob alone does not reject a confident decode",
        ok,
        f"ok={ok} reason={reason!r}",
    )
    ...
    check(
        "low confidence with high no_speech is rejected as silence",
        not ok and "silence" in reason,
        f"ok={ok} reason={reason!r}",
    )
    ...
    check(
        "single segment passes its metrics through",
        t.text == "hello"
        and abs(t.avg_logprob + 0.4) < 1e-9
        and abs(t.no_speech_prob - 0.1) < 1e-9,
        f"{t}",
    )
    ...
    check(
        "avg_logprob is weighted by segment duration",
        abs(t.avg_logprob - (-0.4)) < 1e-9,
        f"avg_logprob={t.avg_logprob}",
    )

    # no_speech_prob is the max across segments (most conservative).
    check(
        "no_speech_prob takes the max across segments",
        abs(t.no_speech_prob - 0.2) < 1e-9,
        f"no_speech_prob={t.no_speech_prob}",
    )
    ...
    ok, _ = confident(t)
    check(
        "no segments yields empty text that fails the guard",
        t.text == "" and not ok,
        f"{t}",
    )
    ...
    check(
        "zero-duration segments do not divide by zero",
        abs(t.avg_logprob - (-0.5)) < 1e-9,
        f"avg_logprob={t.avg_logprob}",
    )
    ...
    on = {
        ...
        "stt_min_avg_logprob": FLOOR,
        "stt_max_no_speech_prob": CEIL,
    }
    ok, _ = pipeline.guard_transcript(Transcript("go", -0.2, 0.05), on)
    check("guard_transcript passes a confident transcript when enabled", ok, f"ok={ok}")

    ok, reason = pipeline.guard_transcript(Transcript("garble", -3.0, 0.05), on)
    check(
        "guard_transcript rejects a low-confidence transcript when enabled",
        not ok and "avg_logprob" in reason,
        f"ok={ok} reason={reason!r}",
    )
    ...
    check(
        "guard_transcript passes everything when the guard is disabled",
        ok and reason == "guard disabled",
        f"ok={ok} reason={reason!r}",
    )
    ...
    for key, want in (
        ...
        check(
            f"DEFAULTS carries {key}",
            pipeline.DEFAULTS.get(key) == want,
            f"{key}={pipeline.DEFAULTS.get(key)!r}",
        )

    cfg = pipeline.load_config()
    check(
        "load_config surfaces the guard keys",
        all(
            k in cfg
            for k in (
