...
Both live loops write stage and timing records to stderr. Wake scores, spoken
transcripts, and replies appear only at `DEBUG`; the default `INFO` level does
not retain that content, and the built-in handler does not write a log file.
`live_driver.py --debug` overrides `log_level` for that process and adds
per-frame wake telemetry.
