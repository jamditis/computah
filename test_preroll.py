...
def main() -> int:
    ...
    fired_frame_value = (
        fire_on - 1
    )  # 0-based: the fire_on-th predict is frame index fire_on-1
    check("detector fires and returns the peak score", score == 1.0, f"score={score}")
    check(
        "pre-roll holds exactly _PREROLL_FRAMES recent frames",
        len(preroll) == n,
        f"len={len(preroll)} want {n}",
    )
    check(
        "pre-roll ends with the firing frame (the consumed-during-detection frame)",
    ...
    check(
        "pre-roll starts _PREROLL_FRAMES back from the firing frame",
    ...
    check(
        "the warm model is reset exactly once for the whole stream, not per frame",
        warm.reset_calls == 1,
        f"reset_calls={warm.reset_calls} want 1",
    )
    check(
        "every frame is scored against that one warm model",
        warm.calls == n_frames,
        f"predict calls={warm.calls} want {n_frames}",
    )
    ...
    check(
        "captured request includes the pre-roll frames",
        nframes(cap) == 18,
        f"got {nframes(cap)} frames, want 18 (3 seed + 15)",
    )
    check(
        "pre-roll frames sit at the very front, in order",
    ...
    check(
        "abandoned wake stays empty even with a pre-roll seed",
        cap_empty.size == 0,
        f"got {cap_empty.size} samples, want 0",
    )
    ...
    for k in (1, 2):
        cap_transient = pipeline.capture_request(
            iter(loud(k) + silent(30)), preroll=loud(3)
        )
        check(
            f"a {k}-frame transient after the wake stays empty (no phantom command)",
            cap_transient.size == 0,
            f"got {cap_transient.size} samples, want 0",
        )
    ...
    check(
        "a sustained 3-frame command still captures with its pre-roll",
        nframes(cap_short) == 15,
        f"got {nframes(cap_short)} frames, want 15 (2 seed + 3 + 10)",
    )
    ...
    check(
        "no pre-roll behaves like the original capture",
        nframes(cap_none) == 15,
    ...
    try:
        ...
        cap_ok = pipeline.capture_request(
            iter(loud(5) + silent(15)), preroll=loud(2), vad_threshold=0.5
        )
        check(
            "confirmed speech captures and keeps its pre-roll",
            nframes(cap_ok) == 17,
            f"got {nframes(cap_ok)} frames, want 17 (2 seed + 5 + 10)",
        )

        pipeline._confirm_speech = lambda pcm, threshold: False
        cap_noise = pipeline.capture_request(
            iter(loud(5) + silent(15)), preroll=loud(2), vad_threshold=0.5
        )
        check(
        ...
        seen = {}
        pipeline._confirm_speech = lambda pcm, threshold: (
            seen.update(n=len(pcm)) or True
        )
        pipeline.capture_request(
            iter(loud(5) + silent(15)), preroll=loud(2), vad_threshold=0.5
        )
        check(
            "VAD sees only the post-fire command audio, not the pre-roll",
        ...
        pipeline._confirm_speech = lambda pcm, threshold: (
            False
        )  # would reject if consulted
        cap_skip = pipeline.capture_request(iter(loud(5) + silent(15)), preroll=loud(2))
        check(
            "no vad_threshold skips the VAD gate (energy-only path unchanged)",
            nframes(cap_skip) == 17,
            f"got {nframes(cap_skip)} frames, want 17",
        )
    finally:
    ...
    got = seen["preroll"]
    check(
        "run_turn passes the frames gathered during detection to capture_request",
