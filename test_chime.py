...
def test_generator() -> None:
    ...
    check(
        "cue length is in a sane 80-600 ms range",
        80.0 <= ms <= 600.0,
    ...
    check(
        "edges fade to near silence (no click)", edge < 500, f"max edge sample={edge}"
    )
    ...
    check(
        "pitch rises (second tone higher than first)",
        hi > lo + 50,
    ...
    check(
        "wake_cue_wav writes a 16 kHz mono WAV at the given path",
    ...
    check(
        "wake_cue_wav is idempotent (same path, file intact)",
        p2 == path and os.path.getmtime(path) == mtime,
        f"p2={p2!r}",
    )
...
def _run_turn_order(
    cfg_wake: float, fire: bool, use_hook: bool = True, hook_boundary: bool = True
):
...
def test_run_turn_hook() -> None:
    ...
    check(
        "on_wake fires once, after detection and before capture",
        order == ["detect", "wake", "capture"],
        f"order={order}",
    )
    check(
        "a cue that played (hook returns True) drops the pre-roll so the wake-word "
        "tail is not prepended",
        pr == [[]],
        f"preroll lengths seen by capture={[len(p) for p in pr]}",
    )

    order, pr = _run_turn_order(0.5, fire=True, use_hook=True, hook_boundary=False)
    check(
        "a cue that failed (hook returns falsy, no boundary) keeps the pre-roll so "
        "a no-pause command is still recovered (issue #30)",
        order == ["detect", "wake", "capture"] and len(pr) == 1 and len(pr[0]) == 1,
        f"order={order} preroll lengths={[len(p) for p in pr]}",
    )

    order, pr = _run_turn_order(0.5, fire=True, use_hook=False)
    check(
        "without the chime hook the pre-roll is preserved (issue #30 recovery)",
    ...
    check(
        "on_wake does not fire when no wake fires",
        order == ["detect"] and "wake" not in order,
        f"order={order}",
    )
...
def test_live_driver_chime() -> None:
    ...
    try:
        ran = live_driver.run_turn(
            iter([]), mic, object(), 0.5, os.devnull, None, cfg, False
        )
    finally:
        (
            live_driver.listen_for_wake,
    ...
    check(
        "chime plays before the request is captured",
        "play:cue" in order
        and "capture" in order
        and order.index("play:cue") < order.index("capture"),
        f"ran={ran} order={order}",
    )
    check(
        "reply still plays after capture",
        "play:reply" in order and order.index("play:reply") > order.index("capture"),
        f"order={order}",
    )
    check(
        "a cue that played flushes-to-now once (drops the post-cue mic bleed)",
        mic.flushed == 1,
        f"flushed={mic.flushed}",
    )
    check(
        "the chime drops the pre-roll so the wake-word tail is not prepended",
        seen_preroll == [[]],
...
def test_live_driver_chime_failure_keeps_preroll() -> None:
    ...
    check(
        "a failed cue keeps the pre-roll so a no-pause command is still recovered",
        len(seen_preroll) == 1 and len(seen_preroll[0]) == 1,
        f"preroll lengths seen by capture={[len(p) for p in seen_preroll]}",
    )
    check(
        "the turn still completes after a failed cue",
        ran is True and "capture" in order,
        f"ran={ran} order={order}",
    )
...
def test_chime_opt_in_default_off() -> None:
    ...
    for label, cfg_extra in (
        ...
        check(
            f"no cue and no flush when {label}",
            "play:cue" not in order and mic.flushed == 0,
            f"ran={ran} order={order} flushed={mic.flushed}",
        )
        check(
            f"the turn still runs (capture then reply) when {label}",
            "capture" in order and "play:reply" in order,
            f"order={order}",
        )
        check(
            f"the pre-roll is preserved (not chime-cleared) when {label}",
            len(seen_preroll) == 1 and len(seen_preroll[0]) == 1,
            f"preroll lengths seen by capture={[len(p) for p in seen_preroll]}",
        )
