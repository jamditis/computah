...
def main() -> int:
    ...
    check("800 ms is 10 frames", pipeline._ms_to_frames(800) == 10,
          f"got {pipeline._ms_to_frames(800)}")
    check("8000 ms is 100 frames", pipeline._ms_to_frames(8000) == 100,
          f"got {pipeline._ms_to_frames(8000)}")
    check("a sub-frame value floors to 1, never 0 frames",
          pipeline._ms_to_frames(10) == 1, f"got {pipeline._ms_to_frames(10)}")
    ...
    check("endpoint_silence_ms default maps to _ENDPOINT_SILENCE_FRAMES",
    ...
    check("max_request_ms default maps to _MAX_REQUEST_FRAMES",
    ...
    explicit_none = pipeline.capture_request(iter(loud(5) + silent(15)),
                                             endpoint_silence_ms=None, max_request_ms=None)
    check("no keys captures 5 speech + 10 endpoint = 15 frames", nframes(base) == 15,
          f"got {nframes(base)}")
    check("explicit None matches the no-keys default", nframes(explicit_none) == 15,
    ...
    short = pipeline.capture_request(iter(loud(5) + silent(15)), endpoint_silence_ms=400)
    check("a 400 ms window endpoints after 5 quiet frames (10 total)",
          nframes(short) == 10, f"got {nframes(short)}, want 10 (5 speech + 5 quiet)")
    # 1200 ms -> 15 frames: needs more trailing quiet, so it holds longer; 5 + 15 = 20.
    long = pipeline.capture_request(iter(loud(5) + silent(20)), endpoint_silence_ms=1200)
    check("a 1200 ms window holds longer (20 total)",
          nframes(long) == 20, f"got {nframes(long)}, want 20 (5 speech + 15 quiet)")
    ...
    check("a 800 ms cap stops unbroken speech at 10 frames",
          nframes(capped) == 10, f"got {nframes(capped)}, want 10")
    check("the cap is well under the built-in default of "
          f"{pipeline._MAX_REQUEST_FRAMES}",
    ...
    seen: dict = {}
    real = (pipeline.load_config, pipeline.stream_detect_wake, pipeline.capture_request,
    ...
    pipeline.load_config = lambda: {"wake_word": "hey_jarvis", "wake_threshold": 0.5,
                                    "capture_vad_threshold": 0.5,
                                    "endpoint_silence_ms": 440, "max_request_ms": 2000}
    ...
    try:
        r = pipeline.run_turn(iter(loud(1)), out_wav_path=str(TEST_DIR / "endpoint_reply.wav"))
    finally:
        (pipeline.load_config, pipeline.stream_detect_wake, pipeline.capture_request,
         pipeline.transcribe_detailed, pipeline.guard_transcript,
         pipeline.brain, pipeline.speak,
         pipeline._get_oww_model, pipeline._resolve_wake_path) = real
    check("pipeline.run_turn passes cfg endpoint_silence_ms to capture_request",
    ...
    check("pipeline.run_turn passes cfg max_request_ms to capture_request",
    ...
    seen_ld: dict = {}
    real_ld = (live_driver.listen_for_wake, pipeline.capture_request,
               pipeline.transcribe_detailed, pipeline.guard_transcript,
               pipeline.brain, pipeline.speak, live_driver._play_wav)

    def fake_capture_ld(fr, preroll=None, vad_threshold=None,
                        endpoint_silence_ms=None, max_request_ms=None):
    ...
    ld_cfg = {"capture_vad_threshold": 0.5,
              "endpoint_silence_ms": 640, "max_request_ms": 5000}
    try:
        ld_ran = live_driver.run_turn(iter([]), _FakeMic(), object(), 0.5,
                                      "/dev/null", None, ld_cfg, False)
    finally:
        (live_driver.listen_for_wake, pipeline.capture_request,
         pipeline.transcribe_detailed, pipeline.guard_transcript,
         pipeline.brain, pipeline.speak, live_driver._play_wav) = real_ld
    check("live_driver.run_turn passes cfg endpoint_silence_ms to capture_request",
    ...
    check("live_driver.run_turn passes cfg max_request_ms to capture_request",
